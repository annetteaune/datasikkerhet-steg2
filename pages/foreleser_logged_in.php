...
<head>
    ...
    <title>Foreleser Dashboard - HearMeOut</title>
...
<body>
    ...
    try {
        ...
        if (!$stmt) {
            throw new Exception("Feil ved forberedelse av lecturer_profile_view");
        }
        ...
        // Hent foreleserens emner
        $stmt = $conn->prepare("CALL lecturer_courses_view(?)");
        if (!$stmt) {
            throw new Exception("Feil ved forberedelse av lecturer_courses_view");
        }
    ...
    <main>
        ...
        <div class="dashboard-container">
            ...
            <section class="dashboard-section">
                <h2>Opprett nytt emne</h2>
                <form action="../lib/create_subject.php" method="post" class="create-subject-form">
                    <div class="form-group">
                        <label class="form-label" for="emne_navn">Emne navn</label>
                        <input class="form-input" type="text" id="emne_navn" name="emne_navn" required 
                               placeholder="F.eks. Programmering 101" maxlength="100">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="emne_kode">Emne kode</label>
                        <input class="form-input" type="text" id="emne_kode" name="emne_kode" required 
                               placeholder="F.eks. DAT100" pattern="[A-Z]{3}[0-9]{3}" 
                               title="Emne kode må bestå av 3 store bokstaver fulgt av 3 tall">
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="pin_kode">PIN-kode</label>
                        <input class="form-input" type="text" id="pin_kode" name="pin_kode" required 
                               placeholder="Velg en 4-sifret PIN-kode" pattern="[0-9]{4}" 
                               title="PIN-koden må bestå av 4 siffer">
                    </div>

                    <button type="submit" class="form-submit">Opprett emne</button>
                </form>
            </section>
        </div>
    </main>
