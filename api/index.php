...
$basePath = '/var/www/HearMeOut/DatasikkerhetGruppeProsjekt2025';
require_once $basePath . '/api/config/config.php';
require_once $basePath . '/api/includes/Database.php';
require_once $basePath . '/api/includes/API.php';
    ...
    if ($method === 'GET') {
        switch ($endpoint) {
            ...
            case 'course':
                ...
                default:
                // If no endpoint is specified, show the API documentation
                if (empty($endpoint)) {
                    // Fix the path to api.html
                    header_remove('Content-Type');
                    header('Content-Type: text/html');
                    $apiDoc = file_get_contents($basePath . '/api/api.html');
                    echo $apiDoc;
                    exit;
                }
                
                // Otherwise show the default JSON response
                echo json_encode([
                    'status' => 'success',
