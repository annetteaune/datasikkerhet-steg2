...
try {
    ...
    error_log("Prosesserte input: fornavn='$fornavn', etternavn='$etternavn', 
        epost='$epost', emne_navn='$emne_navn', emne_kode='$emne_kode', pin_kode='$pin_kode'");

    // Grunnleggende validering
    if (
        empty($fornavn) || empty($etternavn) || empty($epost) || empty($passord) ||
        empty($bekreft_passord) || empty($emne_navn) || empty($emne_kode) || empty($pin_kode)
    ) {
        $missing_fields = [];
        if (empty($fornavn)) {
            $missing_fields[] = 'fornavn';
        }
        if (empty($etternavn)) {
            $missing_fields[] = 'etternavn';
        }
        if (empty($epost)) {
            $missing_fields[] = 'epost';
        }
        if (empty($passord)) {
            $missing_fields[] = 'passord';
        }
        if (empty($bekreft_passord)) {
            $missing_fields[] = 'bekreft_passord';
        }
        if (empty($emne_navn)) {
            $missing_fields[] = 'emne_navn';
        }
        if (empty($emne_kode)) {
            $missing_fields[] = 'emne_kode';
        }
        if (empty($pin_kode)) {
            $missing_fields[] = 'pin_kode';
        }
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO foreleser 
            (fornavn, etternavn, epost, passord, bilde) VALUES (?, ?, ?, ?, ?)");
        if (!$stmt) {
            throw new Exception("Database query failed: " . $conn->error);
        }
